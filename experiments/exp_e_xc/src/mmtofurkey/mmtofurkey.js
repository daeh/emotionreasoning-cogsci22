export function mmtofurkey(turk, window, document) {
			...
			if (!!linkFunction) {
				for (var prop in linkFunction) {
					if (linkFunction.hasOwnProperty(prop)) {
						if (unwrap) {
							linkFunction[prop](rawData, turk);
						} else {
							linkFunction[prop](rawData.data, turk);
						}
					}
				}
			}
		} catch (e) {
			console.log("error calling LinkFunction:".e);
		}
