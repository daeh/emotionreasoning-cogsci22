...
(function() {
  ...
  if (document.scripts) {
    for (var i = 0, ii = document.scripts.length; i < ii; i++) {
      ...
      if (/mmtofurkey/.test(scriptSrc) && /\?nowarn/.test(scriptSrc)) {
  ...
  function isTable(array, equality) {
    ...
    if (array.reduce(function(acc, x) {
        return !(x instanceof Object) || acc
      }, false)) {
      return false;
    }
  ...
  var htmlifyTable = function(array) {
    var getRow = function(obj) {
      return tag("tr",
        lines(keys.map(function(k) {
          return tag("td", htmlify(obj[k]))
        })))
    }
    ...
    return tag("table class=tabular valign=top border=1",
      ...
      tag("tr", lines(keys.map(function(k) {
        return tag("th", k)
      }))),
  ...
  turk.submit = function(data, unwrap, linkFunction) {
    var linkFunction;
    linkFunction = linkFunction || false;
    ...
    // Do other things with data before submitting it to mturk
    try {
      if (!!linkFunction) {
        for (var prop in linkFunction) {
          if (linkFunction.hasOwnProperty(prop)) {
            if (unwrap) {
              try {linkFunction[prop](rawData, turk);} catch(err) {console.log('Linker Error: ', err);}
            } else {
              try {linkFunction[prop](rawData.data, turk);} catch(err) {console.log('Linker Error: ', err);}
            }
          }
        }
      }
    } catch (e) { console.log("error calling LinkFunction:".e); }
  ...
  if (showPreviewWarning && turk.previewMode) {
    var intervalHandle = setInterval(function() {
      try {
        ...
        div.innerHTML = "PREVIEW MODE: CLICK \"ACCEPT\" TO START THIS HIT";
